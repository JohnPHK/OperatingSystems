...
typedef struct a1fs_superblock {
	...
	//TODO
	uint64_t inodes_count;   		/* total number of inodes table blocks */
	uint64_t blocks_count;			/* total number of data blocks */
	uint64_t free_blocks_count;		/* total number of unsed data blocks */
	uint64_t free_inodes_count;		/* total number of unsed inodes table blocks */
	uint64_t inode_bitmap;			/* the starting block of the inode bitmap block */
	uint64_t datablock_bitmap;		/* the starting block of the data bitmap block */
	uint64_t first_inode_block;		/* the starting block of the inode table block */
	uint64_t first_data_block;		/* the starting block of the data block */
...
typedef struct a1fs_inode {
	...
	struct timespec	ctime;	          /* Inode Creation time */
	struct timespec	dtime;            /* Deletion time */
	uint16_t uid;         	          /* who owns this file */
	uint16_t gid;			          /* which group does this file belong to */
	uint32_t i_blocks;			      /* how many blocks have been allocated to this file */
	a1fs_extent i_block[15];		  /* Pointers to blocks */
	uint32_t extra[12];				  /* Padding */
} a1fs_inode;
